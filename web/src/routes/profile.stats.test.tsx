...
test("renders the picked user's panels", async () => {
  ...
  render(wrap(<ProfileStatsView user="u1" range="30d" library="" onUser={noop} onRange={noop} />));
...
test("switching library refetches the profile detail", async () => {
  const calls: string[] = [];
  vi.spyOn(globalThis, "fetch").mockImplementation((input) => {
    const url = String(input);
    calls.push(url);
  ...
  // Same QueryClient across both renders: a refetch here can only come from
  // the query key picking up the new library, not from a remounted client.
  const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
  const { rerender } = render(
    <QueryClientProvider client={qc}>
      <ProfileStatsView user="u1" range="30d" library="Movies" onUser={noop} onRange={noop} />
    </QueryClientProvider>,
  );
  await waitFor(() => expect(screen.getByText(/Alpha/)).toBeInTheDocument());
  calls.length = 0;
  ...
  rerender(
    <QueryClientProvider client={qc}>
      <ProfileStatsView user="u1" range="30d" library="TV" onUser={noop} onRange={noop} />
    </QueryClientProvider>,
  );
  await waitFor(() =>
    expect(calls.some((u) => u.includes("/api/profile/u1") && u.includes("library=TV"))).toBe(true),
  );
});
