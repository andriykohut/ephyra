...
// ids' library names after the fact (the watch job's residual backfill sweep
// for spine rows the plugin no longer reports; items that never resolve stay
// candidates and are retried on every subsequent run, not just once). A
// future APISource that can't do this simply doesn't implement it — the
